...
| 2026-05-17 | Fast-block tile-count formula written as `2n + ⌈√(2n)⌉ + 1` in spec.md, todo.md, and routing.md code example | Wrong formula — Litinski 2019 §5.3 paper body text says `2n + ⌈√(8n)⌉ + 1`. `√(8n) = 2√(2n)`, so these are materially different values (e.g. n=10: √(2n)→5 gives 26, √(8n)→9 gives 30). The routing.md table already had the correct `√(8n)` formula but the code example below it used `sqrt(2n)`. | Corrected all three doc files and the `fastTileCount()` implementation to use `sqrt(8n)`. Ordering compact ≤ intermediate ≤ fast now holds for all n ≥ 1. |
